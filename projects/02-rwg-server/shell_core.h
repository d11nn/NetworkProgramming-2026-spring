...
inline int parseNumberPipeCount(const std::string &token) {
	int totalCount = 0;
	size_t termStart = 1;
	while (termStart < token.size()) {
		size_t plusPos = token.find('+', termStart);
		std::string term = token.substr(termStart, plusPos - termStart);
		totalCount += std::stoi(term);
...
inline void discardIncomingPipe(ShellState &state, long long targetSegment) {
	auto incomingIt = state.numberedPipes.find(targetSegment);
	if (incomingIt == state.numberedPipes.end()) {
		return;
	}
	closePipe(incomingIt->second);
	state.numberedPipes.erase(incomingIt);
}

inline bool executeShellBuiltin(const Command &command, ShellState &state, int outputFd) {
	const std::string &name = command.argv.empty() ? std::string() : command.argv[0];
	if (name == "setenv") {
		if (command.argv.size() >= 3) {
			state.env[command.argv[1]] = command.argv[2];
		}
		discardIncomingPipe(state, state.executedSegments + 1);
		++state.executedSegments;
		return true;
	}

	if (name == "printenv") {
		if (command.argv.size() >= 2) {
			auto it = state.env.find(command.argv[1]);
			if (it != state.env.end()) {
...
inline LinePlan buildLinePlan(const std::vector<Command> &commands, const ShellState &state) {
	LinePlan linePlan;
	linePlan.commandSegments.resize(commands.size(), 0);
	linePlan.segmentStarts.resize(commands.size(), false);

	long long segmentCursor = state.executedSegments;
	...
	linePlan.segmentCount = segmentCursor - state.executedSegments;
	return linePlan;
}

inline void attachIncomingNumberedPipe(ShellState &state, long long segmentIndex, int &inFd) {
	auto incomingIt = state.numberedPipes.find(segmentIndex);
	if (incomingIt == state.numberedPipes.end()) {
		return;
	}
...
inline void applyChildEnvironment(const ShellState &state) {
	clearenv();
...
inline void closeNumberedPipesInChild(const ShellState &state) {
	for (const auto &entry : state.numberedPipes) {
...
inline void closeExtraFdsInChild(const std::vector<int> &fds) {
	for (int fd : fds) {
		if (fd > STDERR_FILENO) {
			close(fd);
		}
...
inline void closeDuplicatedFd(int fd) {
	if (fd > STDERR_FILENO) {
		close(fd);
	}
}

struct ReusableIoPlan {
...
inline bool prepareIoPlan(const Command &command, ShellState &state, long long segmentIndex,
						  ReusableIoPlan &ioPlan, int &inFd, const ExecutionConfig &config,
						  bool isFirstCommand, bool isLastCommand) {
	ioPlan.outFd = config.outputFd;
	ioPlan.errFd = config.errorFd;

	if (isFirstCommand && config.inputFd >= 0) {
		closeFd(inFd);
		inFd = config.inputFd;
	}

	if (isLastCommand && config.forceLastOutput && config.outputFd >= 0) {
		ioPlan.outFd = config.outputFd;
		return true;
	}
	...
	PipeFD &targetPipe = state.numberedPipes[targetSegment];
...
[[noreturn]] inline void runChildProcess(const Command &command, const ShellState &state,
										 const ReusableIoPlan &ioPlan, int inFd,
	...
	if (ioPlan.errFd >= 0 && ioPlan.errFd != STDERR_FILENO) {
		dup2(ioPlan.errFd, STDERR_FILENO);
	}

	closeNumberedPipesInChild(state);
	closeExtraFdsInChild(config.closeInChild);
	...
	closeFd(redirectFd);
	closeDuplicatedFd(inFd);
	closeDuplicatedFd(ioPlan.outFd);
	if (ioPlan.errFd != ioPlan.outFd) {
		closeDuplicatedFd(ioPlan.errFd);
	}

	applyChildEnvironment(state);
...
inline void closeParentIo(const ReusableIoPlan &ioPlan) {
	int ordinaryWriteFd = ioPlan.ordinaryPipe.writeFd;
	int redirectFd = ioPlan.redirectFd;
	closeFd(ordinaryWriteFd);
	closeFd(redirectFd);
...
inline void executeExternalCommands(const std::vector<Command> &commands, ShellState &state,
									const ExecutionConfig &config) {
	LinePlan linePlan = buildLinePlan(commands, state);
	std::vector<bool> waitable = buildWaitability(commands);
	std::vector<pid_t> waitList;
	std::vector<pid_t> backgroundList;
	int inFd = -1;
	...
	for (size_t i = 0; i < commands.size(); ++i) {
		...
		if (linePlan.segmentStarts[i]) {
			attachIncomingNumberedPipe(state, segmentIndex, inFd);
		}

		ReusableIoPlan ioPlan;
		if (!prepareIoPlan(command, state, segmentIndex, ioPlan, inFd, config,
						   i == 0, i + 1 == commands.size())) {
			continue;
		}

		pid_t childPid = forkOrReapUntilSuccess();
		if (childPid == 0) {
			runChildProcess(command, state, ioPlan, inFd, config);
		}
	...
	state.executedSegments += linePlan.segmentCount;
}

inline void closeShellState(ShellState &state) {
	for (auto &entry : state.numberedPipes) {
		closePipe(entry.second);
	}
	state.numberedPipes.clear();
}
